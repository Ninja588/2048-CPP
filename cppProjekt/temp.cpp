...
void playBackgroundMusic(const char* filePath) {
    ...
    PlaySoundW(wideFilePath, NULL, SND_ASYNC | SND_LOOP);
...
void renderGridAndTiles() {
    ...
    for (int i = 0; i <= GRID_SIZE; i++) {
        ...
        glVertex2f(GRID_SIZE * tileSize, i * tileSize);
        glVertex2f(i * tileSize, 0);
        glVertex2f(i * tileSize, GRID_SIZE * tileSize);
    }
    glEnd();
    glPopMatrix();
    ...
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            ...
            glBegin(GL_LINES);
                glVertex2f(0, 0);
                glVertex2f(0, GRID_SIZE*tileSize);
                glVertex2f(0, GRID_SIZE*tileSize);
                glVertex2f(GRID_SIZE*tileSize, GRID_SIZE*tileSize);
                glVertex2f(GRID_SIZE*tileSize, GRID_SIZE*tileSize);
                glVertex2f(GRID_SIZE*tileSize, 0);
                glVertex2f(GRID_SIZE*tileSize, 0);
                glVertex2f(0, 0);
            glEnd();
            glPopMatrix();
...
void display3() {
    ...
    drawBitmapString(temp3,633.6/Settings.resolutionWidth,693.2/Settings.resolutionHeight);
